...
class BuildMapViewModel: NSObject, ObservableObject {
    ...
    // NEW: transient highlight tracking for “just-added” items
    @Published var recentlyAddedIDs: Set<UUID> = []
    ...
    // MARK: - NEW: highlight bookkeeping
    private func markRecentlyAdded(_ id: UUID) {
        recentlyAddedIDs.insert(id)
        // Auto-clear after a short showcase window so the pulse disappears
