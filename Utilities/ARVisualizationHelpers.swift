...
// MARK: - NEW: Glowing Destination Star (optionally inside a faint ring)
extension ARVisualizationHelpers {
    /// Creates a bright spinning star marker with optional faint ring shell, ground halo, and label.
    /// - Parameters:
    ///   - position: world position
    ///   - color: base neon color
    ///   - labelText: optional caption above
    ///   - includeRingShell: show a subtle ring around the star (defaults true for readability)
    ///   - starSize: approximate tip-to-tip diameter in meters
    ///   - thickness: extrusion thickness of the star
    ///   - spinSeconds: time for one full spin (lower = faster)
    static func createDestinationStarMarker(
        at position: simd_float3,
        color: UIColor,
        labelText: String? = "Destination",
        includeRingShell: Bool = true,
        starSize: CGFloat = 0.32,
        thickness: CGFloat = 0.014,
        spinSeconds: TimeInterval = 8.0
    ) -> SCNNode {
        let root = SCNNode()
        
        // --- Star core (extruded 5-point star) ---
        let starPath = makeStarPath(points: 5, outerRadius: starSize/2, innerRadius: starSize/4.4)
        let starShape = SCNShape(path: starPath, extrusionDepth: thickness)
        let coreMat = SCNMaterial()
        coreMat.lightingModel = .constant
        coreMat.diffuse.contents = color.withAlphaComponent(0.98)
        coreMat.emission.contents = color
        coreMat.blendMode = .add
        coreMat.writesToDepthBuffer = false
        if #available(iOS 15.0, *) { coreMat.emission.intensity = 1.85 }
        starShape.materials = [coreMat]
        
        let starNode = SCNNode(geometry: starShape)
        // Keep it upright slightly above ground so the halo reads well
        starNode.position.y = 0.015
        starNode.castsShadow = false
        root.addChildNode(starNode)
        
        // Soft glow shell (slightly larger)
        let shellGeom = (starShape.copy() as! SCNGeometry)
        let shellMat = coreMat.copy() as! SCNMaterial
        shellMat.diffuse.contents  = color.withAlphaComponent(0.20)
        shellMat.emission.contents = color.withAlphaComponent(0.30)
        if #available(iOS 15.0, *) { shellMat.emission.intensity = 1.95 }
        shellGeom.materials = [shellMat]
        let shellNode = SCNNode(geometry: shellGeom)
        shellNode.scale = SCNVector3(1.10, 1.10, 1.0)
        root.addChildNode(shellNode)
        
        // Optional faint ring shell (helps visibility from afar)
        if includeRingShell {
            let torus = SCNTorus(ringRadius: starSize * 0.62, pipeRadius: thickness * 0.9)
            let ringMat = coreMat.copy() as! SCNMaterial
            ringMat.diffuse.contents  = color.withAlphaComponent(0.25)
            ringMat.emission.contents = color.withAlphaComponent(0.35)
            if #available(iOS 15.0, *) { ringMat.emission.intensity = 1.9 }
            torus.materials = [ringMat]
            let ring = SCNNode(geometry: torus)
            ring.eulerAngles = SCNVector3(Float.pi / 2, 0, 0) // vertical ring facing camera
            root.addChildNode(ring)
            
            // Give the ring a slow counter-rotation for a subtle kinetic feel
            let ringRot = SCNAction.rotateBy(x: 0, y: -CGFloat.pi*2, z: 0, duration: spinSeconds * 1.5)
            ring.runAction(.repeatForever(ringRot))
        }
        
        // Ground halo for "puddle of light"
        let halo = ARVisualizationHelpers.makeGroundHalo(color: color, sizeMeters: starSize * 1.6)
        halo.position = SCNVector3(0, -0.02, 0)
        root.addChildNode(halo)
        
        // Gentle breathing pulse
        let pulse = SCNAction.sequence([
            .customAction(duration: 0.9) { _, _ in
                if #available(iOS 15.0, *) { coreMat.emission.intensity = 2.05 }
            },
            .fadeOpacity(to: 0.95, duration: 0.9),
            .customAction(duration: 0.9) { _, _ in
                if #available(iOS 15.0, *) { coreMat.emission.intensity = 1.85 }
            },
            .fadeOpacity(to: 1.00, duration: 0.9)
        ])
        root.runAction(.repeatForever(pulse))
        
        // Spin the star in place (around its local Y)
        let spin = SCNAction.rotateBy(x: 0, y: CGFloat.pi * 2, z: 0, duration: spinSeconds)
        starNode.runAction(.repeatForever(spin))
        shellNode.runAction(.repeatForever(spin))
        
        // Optional label
        if let labelText {
            let label = makeFloatingLabel(text: labelText, textColor: .white, bgOpacity: 0.9)
            label.position = SCNVector3(0, Float(starSize * 0.9), 0)
    ...
    /// Builds a star-shaped UIBezierPath centered at origin, pointing "up".
    private static func makeStarPath(points: Int, outerRadius: CGFloat, innerRadius: CGFloat) -> UIBezierPath {
        let path = UIBezierPath()
        let total = points * 2
